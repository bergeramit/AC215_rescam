...
def setup_genai():
    ...
    if not api_key:
        raise ValueError("GEMINI_API_KEY not set. Put it in .env or export it.")
...
def main():
    ...
    parser.add_argument("--tuning-job-name", required=False)
    ...
    def run_model_via_api(model_name: str):
        # Only works for public models like gemini-2.0-flash-001
        # Vertex models (projects/...) need endpoint access
        if model_name.startswith("projects/"):
            print(f"⚠️  Skipping {model_name} - needs endpoint access")
